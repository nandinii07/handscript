/* Upload screen: three slots, then POST /api/jobs.
   ...
   Adapted from app/static/js/upload.js for static hosting: the only
   differences are the backend call going through window.API_BASE (this page
   and the API are no longer necessarily the same origin) and the redirect
   after a successful build using a query string instead of a path segment,
   since this static site has no server-side routing. */
...
(function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    fetch(window.API_BASE + "/api/jobs", { method: "POST", body: payload })
      ...
      .then(function (result) {
        if (result.status === 202 && result.body.job_id) {
          window.location.href = "build.html?job=" + result.body.job_id;
          return;
        }
